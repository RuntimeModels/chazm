...
import model.organization.relation.AchievesFactory;
...
public class AchievesEventTest {
	...
	public void testAchievesEvent() {
		...
		final AchievesEvent ae1 = aef.build(a, EventCategory.ADDED);
		final AchievesEvent ae2 = aef.build(a, EventCategory.ADDED);
	...
	public void testAchievesEvent2() {
		...
		new AchievesEvent(null, null);
	}

	@Test
	public void testAchievesEvent3() {
		exception.expect(instanceOf(ProvisionException.class));
		exception.expectMessage(allOf(containsString("parameter"), containsString(".<init>()"), containsString("is not @Nullable")));

		aef.build(null, EventCategory.ADDED);
	}

	@Test
	public void testAchievesEvent4() {
		exception.expect(instanceOf(IllegalArgumentException.class));
		exception.expectMessage(equalTo("Parameter (achieves) cannot be null"));

		new AchievesEvent(null, EventCategory.ADDED);
	}
	...
	public void testGetRoleId() {
		...
		final SpecificationGoal sg = sgf.buildSpecificationGoal(idf.build(SpecificationGoal.class, "sg"));
		final Achieves a = af.buildAchieves(r, sg);
		final AchievesEvent ae = aef.build(a, EventCategory.ADDED);
	...
	public void testEquals() {
		...
		final AchievesEvent ae1 = aef.build(a1, EventCategory.ADDED);
		final AchievesEvent ae2 = aef.build(a2, EventCategory.ADDED);
		final AchievesEvent ae3 = aef.build(a3, EventCategory.ADDED);
