...
/**
 * The <code>ModelFactory</code> class allows the random generation of a organization model based on the current implementation available from the package
...
public class ModelFactory {
	 ...
	 */
	public static Organization generate(final int numberOfGoals, final int numberOfRoles, final int numberOfAgents, final int averageNumberOfRolesPerGoal,
			final int averageNumberOfAgentsPerRole) {
		assert averageNumberOfRolesPerGoal <= numberOfRoles && averageNumberOfAgentsPerRole <= numberOfAgents;
		...
		if (DEBUG) {
			System.out.println("--- Creating Achieves Relation (Goal-Role) ---");
		}
		...
		for (final SpecificationGoal goal : goals) {
			...
			final int max = Math.min(numberOfRoles, averageNumberOfRolesPerGoal * numberOfGoals - totalRolesAssigned - (numberOfGoalsLeft - 1));
			if (DEBUG) {
				System.out.println(String.format("  Range: %s ... %s", min, max));
			}
			/*
			 * once the range (min .. max) is computed, pick a number randomly from that range as the number of roles that will be assigned to the current goal
			 */
			final int rolesToAssign = (int) Math.round(Math.random() * (max - min) + min);
			totalRolesAssigned += rolesToAssign;
			numberOfGoalsLeft--;
			if (DEBUG) {
				System.out.println(String.format("    # Roles to Assign = %s (Total Assigned: %s)", rolesToAssign, totalRolesAssigned));
				System.out.println(String.format("    Goals Left = %s", numberOfGoalsLeft));
			}
			/*
			 * once we have the number of roles to be assigned to the current goal, we can go ahead and create the achieves relationship
			 */
			if (DEBUG) {
			...
			for (int i = 0; i < rolesToAssign; i++) {
				...
				organization.addAchievesRelation(role.getId(), goal.getId());
				if (DEBUG) {
		...
		if (DEBUG) {
			System.out.println("--- Creating Capable Relation (Role-Agent) ---");
		}
		/* randomizes the roles list */
		Collections.shuffle(roles);
		...
		for (final Role role : roles) {
			...
			final int max = Math.min(numberOfAgents, averageNumberOfAgentsPerRole * numberOfRoles - totalAgentsAssigned - (numberOfRolesLeft - 1));
			if (DEBUG) {
				System.out.println(String.format("  Range: %s ... %s", min, max));
			}
			/*
			 * once the range (min .. max) is computed, pick a number randomly from that range as the number of agents that will be assigned to the current role
			 */
			final int agentsToAssign = (int) Math.round(Math.random() * (max - min) + min);
			totalAgentsAssigned += agentsToAssign;
			numberOfRolesLeft--;
			if (DEBUG) {
				System.out.println(String.format("    Agents to Assign = %s (Total Assigned: %s)", agentsToAssign, totalAgentsAssigned));
				System.out.println(String.format("    Roles Left = %s", numberOfRolesLeft));
			}
			/*
			 * once we have the number of agents to be assigned to the current role, we can go ahead and create the possesses relationship
			 */
			if (DEBUG) {
			...
			for (int i = 0; i < agentsToAssign; i++) {
				...
				for (final Capability capability : role.getRequiresSet()) {
					...
					if (DEBUG) {
						System.out.println(String.format("    %s <-> %s : %s", agent, capability, score));
					}
				}
				for (final Attribute attribute : role.getNeedsSet()) {
					...
					case POSITIVE_QUALITY:
						break;
					default:
						throw new IllegalArgumentException(String.format("Unknown Enum (%s)", attribute.getType()));
					}
					agent.addHas(attribute, value);
					if (DEBUG) {
						System.out.println(String.format("    %s <-> %s : %s", agent, attribute, value));
					}
				}
			}
		}
