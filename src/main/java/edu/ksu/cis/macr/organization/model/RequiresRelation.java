...
public class RequiresRelation {
	 ...
	 * <code>RequiresRelation</code> class.
	 */
	 ...
	 */
	RequiresRelation(final Role role, final Capability capability) {
		if (role == null || capability == null) {
			throw new IllegalArgumentException(String.format(
					"Parameters (role: %s, capability: %s) cannot be null",
					role, capability));
		}
		this.role = role;
	...
	public boolean equals(final Object object) {
		if (object instanceof RequiresRelation) {
			...
			return getRole().equals(requiresRelation.getRole())
					&& getCapability().equals(requiresRelation.getCapability());
		}
		return false;
	}
